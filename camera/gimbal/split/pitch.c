...
#define SERVO_PIN 1   // wiringPi 1 = GPIO18, physical pin 12 (was servo_test/gimbal/serve_test_gpt.c)
...
int main(void) {
    ...
    (void)roll_center;
    softPwmWrite(SERVO_PIN, pitch_center);
    ...
    while (1) {
        ...
        int duty = angleToDuty(pitch_deg, pitch_center);
        softPwmWrite(SERVO_PIN, duty);
        printf("pitch=%.1fdeg -> duty=%d\n", pitch_deg, duty);
    }
